...
public class QRCodeScannerController: UIViewController,
    ...
    var scannerHeight: CGFloat = 240.0
    ...
    private func addRoundCornerFrame() {
        ...
        addLineImage(frame: CGRect(origin: CGPoint(x: (self.view.frame.midX - width/2) + 5,
                                                           y: (self.view.frame.midY - height/2) + 5),
                                   size: CGSize(width: width - 10, height: 2.0)))
        scannerHeight = qrFramedView.frame.height - 10
        print("scanner height: \(scannerHeight)")
    ...
    private func addLineImage(frame: CGRect) {
        if !qrScannerConfiguration.isLineImage{
            lineImage = UIImageView(frame: frame)
            lineImage!.backgroundColor = .red
    ...
    @objc private func timerAction() {
        ...
        UIView.animate(withDuration: 1.0, animations: {
            if self.counter % 2 == 0{
                self.lineImage!.frame.origin.y -= self.scannerHeight /// go up
            }else{
                self.lineImage!.frame.origin.y += self.scannerHeight/// go down
            }
           })
       }
